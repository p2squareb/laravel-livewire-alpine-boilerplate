<main>
                ...
                <nav class="flex mb-5" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                                <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">댓글 관리</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-lg font-semibold text-gray-900 dark:text-white">댓글 관리</h1>
            </div>
            <div class="sm:flex">
                ...
                <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                    ...
                    <div class="relative w-full">
                        <input type="search" wire:model="searchString" wire:keydown.enter="comments" id="searchString" class="block p-[7px] w-full md:w-[270px] text-sm text-gray-900 bg-gray-50 rounded-md border border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" placeholder="제목+내용 검색"/>
                        <button type="button" wire:click.prevent="comments" class="absolute top-0 end-0 px-3 py-[7px] text-sm font-medium h-full text-white bg-blue-700 rounded-e-lg border border-blue-700 hover:bg-blue-800 focus:outline-none dark:bg-blue-600 dark:hover:bg-blue-700">
                ...
                <div class="overflow-hidden shadow">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="p-4 text-sm font-medium text-center text-gray-500 uppercase dark:text-gray-400">내용</th>
                            <th scope="col" class="p-4 text-sm font-medium text-center text-gray-500 uppercase dark:text-gray-400 w-[100px]">추천</th>
                            <th scope="col" class="p-4 text-sm font-medium text-center text-gray-500 uppercase dark:text-gray-400 w-[120px]">작성자</th>
                            <th scope="col" class="p-4 text-sm font-medium text-center text-gray-500 uppercase dark:text-gray-400 w-[120px]">작성일</th>
                        ...
                        @foreach($comments as $comment)
                            <tr wire:key="{{$comment->id}}" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white">{{ $comment->board->table_name }}</td>
                                <td class="p-4 text-sm text-left @if ($comment->is_delete === 1) text-gray-400 dark:text-gray-500 line-through @else text-gray-900 dark:text-white @endif whitespace-nowrap">
                                    <a href="{{ route('write.read', ['tableId' => $comment->table_id, 'writeId' => $comment->write_id]) }}" target="_blank">
                                        @if ($comment->is_delete === 1)[삭제됨]@endif{!! nl2br($comment->comment) !!}
                                    </a>
                                </td>
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white w-[100px]">
                                    @if ($comment->board->use_rate === 1)
                                        @if ($comment->rate_up - $comment->rate_down > 0)
                                            <div class="text-blue-700 dark:text-blue-500">{{ formatNumberWithK($comment->rate_up - $comment->rate_down) }}</div>
                                        @elseif ($comment->rate_up - $comment->rate_down < 0)
                                            <div class="text-red-700 dark:text-red-600">{{ formatNumberWithK($comment->rate_up - $comment->rate_down) }}</div>
                                        @else
                                            <div class="text-gray-900 dark:text-white">{{ formatNumberWithK($comment->rate_up - $comment->rate_down) }}</div>
                                        @endif
                                    @else
                                        <div class="text-gray-900 dark:text-white">-</div>
                                    @endif
                                </td>
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white w-[120px]">{{ $comment->writer }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white w-[120px]">{{ humanReadableDate($comment->created_at, 1) }}</td>
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white">
                                    @if ($comment->is_delete === 0)
                                        <button
                                            type="button"
                                            @click="$dispatch('open-confirm', {type: 'single-delete', link: '', message: '삭제 처리하시겠습니까?<br>삭제 이후에 복원이 가능합니다.'})" wire:click.prevent="$set('deleteRowId', {{ $comment->id }})"
                                    ...
                                    @else
                                        <button type="button" wire:click.prevent="restoreWrite({{ $comment->id }})" class="inline-flex items-center px-3 py-[7px] text-sm font-medium text-center text-white bg-teal-600 rounded-md hover:bg-teal-800">
                                ...
                                <td class="p-4 text-sm text-center text-gray-900 whitespace-nowrap dark:text-white">
                                    <button
                                        type="button"
                                        @click="$dispatch('open-confirm', {type: 'single-remove', link: '', message: '데이터를 삭제하시겠습니까?<br>삭제된 데이터는 다시 복구 할 수 없습니다.'})" wire:click.prevent="$set('removeRowId', {{ $comment->id }})"
                                        class="inline-flex items-center px-3 py-[7px] text-sm font-medium text-center text-white bg-red-600 rounded-md hover:bg-red-800"
                                    >
    ...
    {{ $comments->links('vendor.livewire.common') }}
